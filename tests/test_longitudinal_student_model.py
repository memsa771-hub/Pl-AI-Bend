...
def test_qualifications_map_across_countries_to_shared_levels():
    # The same canonical stage, reached through four different national systems.
    assert canonical_level("FSc Pre-Engineering") == "higher_secondary"
    assert canonical_level("A-Levels") == "higher_secondary"
    assert canonical_level("CBSE 12") == "higher_secondary"
    assert canonical_level("High School Diploma") == "higher_secondary"

    assert canonical_level("Matriculation") == "secondary"
    assert canonical_level("O-Levels") == "secondary"
    assert canonical_level("BSCS") == "bachelor"
    assert canonical_level("MS Artificial Intelligence") == "master"
    assert canonical_level("PhD in Physics") == "phd"
    assert canonical_level("weekend pottery workshop") is None


def test_qualification_keeps_framework_and_country_metadata():
    fsc = classify_qualification("FSc Pre-Medical")
    assert fsc is not None
    assert (fsc.canonical_level, fsc.framework, fsc.country) == (
        "higher_secondary",
        "HSSC",
        "PK",
    )
    # An internationally used qualification is not pinned to a country.
    a_level = classify_qualification("A-Levels")
    assert a_level is not None
    assert a_level.country is None
...
def test_resolver_attaches_grade_to_the_named_qualification():
    rows = _timeline_rows()
    outcome = resolve_education({"degree": "FSc", "percentage": 82.0}, rows)
...
def test_resolver_never_merges_different_levels():
    ...
    outcome = resolve_education(
        {"degree": "BSCS", "institution": "Punjab College"}, [rows[1]]
    )
    assert outcome.match is None
    assert outcome.ambiguous is False
