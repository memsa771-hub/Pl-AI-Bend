...
def _qualification_text(payload: dict[str, Any], row: Education | None = None) -> str:
    parts = [
        payload.get("degree") or (row.degree if row else None),
        payload.get("original_name") or (row.original_name if row else None),
        payload.get("major") or (row.major if row else None),
    ]
    return " ".join(str(part) for part in parts if part)


def _apply_qualification_identity(row: Education, payload: dict[str, Any]) -> None:
    """Map the qualification onto a canonical level without losing its real name."""
    if not row.original_name and payload.get("degree"):
        row.original_name = str(payload["degree"])[:256]
    spec = classify_qualification(_qualification_text(payload, row))
    if spec is None:
        return
    row.canonical_level = spec.canonical_level
    if spec.framework and not row.framework:
        row.framework = spec.framework
    if spec.country and not row.country:
